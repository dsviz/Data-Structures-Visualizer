...
export type LinkedListTool = 'edit' | 'delete' | null;

interface LinkedListToolsProps {
    activeTool: LinkedListTool;
    setActiveTool: (tool: LinkedListTool) => void;
    onAdd: () => void;
    onClear: () => void;
}

export const LinkedListTools: React.FC<LinkedListToolsProps> = ({
    activeTool,
    setActiveTool,
    onAdd,
    onClear
}) => {
    const tools: { id: LinkedListTool, icon: string, label: string }[] = [
        { id: 'edit', icon: 'edit', label: 'Edit Value' },
        { id: 'delete', icon: 'delete', label: 'Delete Node' },
    ];
